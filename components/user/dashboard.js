...
import { Wallet, Users, CheckCircle, TrendingUp, Share2, Bell, User, Settings } from "lucide-react"
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-amber-800 dark:text-amber-200">Complete KYC Verification</p>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{weeklyProgress.toFixed(0)}%</span>
                </div>
                <Progress value={weeklyProgress} className="h-2" />
                <p className="text-xs text-muted-foreground">
                  ₹{(user.weeklyGoal - user.weeklyEarned).toFixed(2)} left to reach goal
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{referralProgress.toFixed(0)}%</span>
                </div>
                <Progress value={referralProgress} className="h-2" />
                <p className="text-xs text-muted-foreground">
                  {user.referralGoal - user.referralsToday} more referrals needed
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-primary">{user.totalReferrals}</div>
              <div className="text-xs text-muted-foreground">Total Referrals</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-accent">₹{user.totalEarnings.toFixed(0)}</div>
