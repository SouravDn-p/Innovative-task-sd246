...
import { useSelector } from "react-redux"   // ✅ user info নেওয়ার জন্য
...
export function TasksPage() {
  ...
  // ✅ লগইন করা ইউজারের ডেটা নেওয়া হচ্ছে Redux থেকে
  const { user } = useSelector((state) => state.auth)

  // ✅ এখন API কল হবে logged-in user এর email সহ
  const {
    data: tasks = [],
    isLoading,
    error,
    ...
    search: searchTerm,
    status: "available",
    email: user?.email,   // 👈 filter by logged-in user
  })
  ...
  const handleSubmitProof = async (e) => {
    ...
    try {
      await submitTaskProof({
        ...
        proof: proofData,
        email: user?.email,   // 👈 প্রমাণ সাবমিট করার সময়ও email পাঠানো হচ্ছে
      }).unwrap()
